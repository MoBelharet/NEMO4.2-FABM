...
CONTAINS
   ...
   SUBROUTINE p4z_sed( kt, knt, Kbb, Kmm, Krhs )
      ...
      REAL(wp), DIMENSION(jpi,jpj    ) :: zsedcal, zsedsi, zsedc, zwstpoc_diag
      ...
      DO_2D( nn_hls, nn_hls, nn_hls, nn_hls )
         ikt  = mbkt(ji,jj) ! mbkt : bottom ocean k-index of T-level (=1 over land)
      ...
      IF( .NOT.lk_sed ) THEN
       ...
       CALL iom_put("zwstpoc", zwstpoc_diag(:,:) * tmask(:,:,1) )
