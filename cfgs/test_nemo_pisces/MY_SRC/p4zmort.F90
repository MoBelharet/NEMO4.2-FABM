...
CONTAINS
   ...
   SUBROUTINE p4z_mort_nano( Kbb, Krhs )
      ...
      REAL(wp), DIMENSION(jpi,jpj,jpk) :: zrespp_, zcompaph_, ztortp_, zlim1_phy
      ...
      DO_3D( nn_hls, nn_hls, nn_hls, nn_hls, 1, jpkm1)
         zcompaph = MAX( ( tr(ji,jj,jk,jpphy,Kbb) - 1.e-9 ), 0.e0 )
         ...
         zrespp  = wchln * 1.e6 * xstep * zlim1 * xdiss(ji,jj,jk) * zcompaph * tr(ji,jj,jk,jpphy,Kbb)

         zcompaph_(ji,jj,jk) = zcompaph

         zlim1_phy(ji,jj,jk) = zlim1
         ...
         !
         ! Mokrane
         zrespp_(ji,jj,jk) = zrespp/xstep
         ztortp_(ji,jj,jk) = ztortp/xstep


      END_3D

      CALL iom_put("phy_zrespp",zrespp_(:,:,:) * tmask(:,:,:))
      CALL iom_put("phy_ztortp",ztortp_(:,:,:) * tmask(:,:,:))
      CALL iom_put("phy_zcompaph",zcompaph_(:,:,:) * tmask(:,:,:))
      CALL iom_put("phy_zlim1_bis"  , zlim1_phy(:,:,:) * tmask(:,:,:))
   ...
   SUBROUTINE p4z_mort_diat( Kbb, Krhs )
      ...
      REAL(wp) ::   zlim2, zlim1
      CHARACTER (len=25) ::   charout
      REAL(wp), DIMENSION(jpi,jpj,jpk) :: zlim1_, ztortp2_, zrespp2_, zcompadi_
      !!---------------------------------------------------------------------
      !
      ...
      DO_3D( nn_hls, nn_hls, nn_hls, nn_hls, 1, jpkm1)
         ...
         zlim1   = 0.25 * ( 1. - zlim2 ) / ( 0.25 + zlim2 ) 
         zrespp2 = wchld * 1.e6 * xstep * zlim1 * xdiss(ji,jj,jk) * zcompadi * tr(ji,jj,jk,jpdia,Kbb)
         
         zlim1_(ji,jj,jk) = zlim1
         ...
         ztortp2_(ji,jj,jk) = ztortp2/xstep
         zrespp2_(ji,jj,jk) = zrespp2/xstep
      END_3D

      CALL iom_put("zlim1_bis",zlim1_(:,:,:) * tmask(:,:,:))
      CALL iom_put("xdiss",xdiss(:,:,:) * tmask(:,:,:))
      CALL iom_put("ztortp2", ztortp2_(:,:,:) * tmask(:,:,:))
      CALL iom_put("zrespp2",zrespp2_(:,:,:) * tmask(:,:,:))
      CALL iom_put("dia_zcompadi",zcompadi_(:,:,:) * tmask(:,:,:))
      !
      IF(sn_cfctl%l_prttrc) THEN      ! print mean trends (used for debugging)
