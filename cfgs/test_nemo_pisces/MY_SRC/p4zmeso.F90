...
CONTAINS
   ...
   SUBROUTINE p4z_meso( kt, knt, Kbb, Kmm, Krhs )
      ...
      REAL(wp), DIMENSION(jpi,jpj,jpk) :: zgrarem, zgraref, zgrapoc, zgrapof, zgrabsi, zfoodlim_, zgraze2_, ztmp1_,ztmp3_, zdiffdn_, &
      & zgraznc_, zgrazdc_, zcompaph_, zcompadi_, ztmp2_,ztmp4_, zcompapoc_, zgrasratn_, zgrazpoc_, zepsherv_, zepsherf_, zepshert_, &
      & zepsherq_, zbeta_, zgrazpof_, zproport_, zgrasratf_, zgraztotf_, zgraztotc_, zgraztotn_
      ...
      DO_3D( nn_hls, nn_hls, nn_hls, nn_hls, 1, jpkm1)
         ...
         !-----Mokrane --------
         zfoodlim_(ji,jj,jk) = zfoodlim
         zcompaph_(ji,jj,jk) = zcompaph
         zcompadi_(ji,jj,jk) = zcompadi
         zcompapoc_(ji,jj,jk) = zcompapoc
         !-------------------------------
         ...
         !-------Mokrane---------------
         zgraze2_(ji,jj,jk) = zgraze2 / xstep
         !--------------------------------
         ...
         !--- Mokrane ---------
         ztmp3_(ji,jj,jk) = ztmp3 ! diatoms
         ztmp1_(ji,jj,jk) = ztmp1 ! nano
         ztmp2_(ji,jj,jk) = ztmp2 ! POC
         ztmp4_(ji,jj,jk) = ztmp4 ! microzoo
         zdiffdn_(ji,jj,jk) = zdiffdn
         !-----------------------
         ...
         zgrazpof_(ji,jj,jk) = zgrazpof / xstep
         ...
         !
         zgraztotc = zgraznc + zgrazdc + zgrazz + zgrazpoc + zgrazffep + zgrazffeg
         ...
         zepsherv_(ji,jj,jk) = zepsherv
         zepsherf_(ji,jj,jk) = zepsherf
         zepshert_(ji,jj,jk) = zepshert
         zepsherq_(ji,jj,jk) = zepsherq
         zbeta_(ji,jj,jk) = zbeta
         zgrasratf_(ji,jj,jk)  = zgrasratf
         zgraztotf_(ji,jj,jk)  = zgraztotf/xstep
         zgraztotc_(ji,jj,jk)  =  zgraztotc/xstep
         zgraztotn_(ji,jj,jk)  =  zgraztotn/xstep
         ! 
         ...
         !zgraref(ji,jj,jk) = ( 1. - unass2 ) * zgraztotf - feratm * zepsherv * zgraztotc &
         ...
         ! Mokrane : 
         zgraznc_(ji,jj,jk)  = zgraznc /xstep
         zgrazdc_(ji,jj,jk)  = zgrazdc /xstep
         zgrazpoc_(ji,jj,jk)  = zgrazpoc /xstep
         !
      END_3D

      !CALL iom_put("ztmp1", ztmp1_(:,:,:) * tmask(:,:,:) ) ! nano
      !CALL iom_put("ztmp3", ztmp3_(:,:,:) * tmask(:,:,:) ) ! diatoms
      CALL iom_put("ztmp2_mes", ztmp2_(:,:,:) * tmask(:,:,:) ) ! POC
      !CALL iom_put("ztmp4", ztmp4_(:,:,:) * tmask(:,:,:) ) ! microzoo
      !CALL iom_put("zfoodlim2", zfoodlim_(:,:,:) * tmask(:,:,:) )
      !CALL iom_put("zgraze2", zgraze2_(:,:,:) * tmask(:,:,:) )
      !CALL iom_put("zdiffdn", zdiffdn_(:,:,:) * tmask(:,:,:) )
      !CALL iom_put("zgraznc", zgraznc_(:,:,:) * tmask(:,:,:) )
      !CALL iom_put("zgrazdc", zgrazdc_(:,:,:) * tmask(:,:,:) )
      CALL iom_put("quotan_mes" , quotan(:,:,:) * tmask(:,:,:) )
      !CALL iom_put("zcompaph_mes" , zcompaph_(:,:,:) * tmask(:,:,:) )
      !CALL iom_put("zcompadi_mes" , zcompadi_(:,:,:) * tmask(:,:,:) )
      CALL iom_put("zcompapoc_mes" , zcompapoc_(:,:,:) * tmask(:,:,:) )
      CALL iom_put("zgrazpoc_mes" , zgrazpoc_(:,:,:) * tmask(:,:,:) )
      CALL iom_put("zgrasratn_mes", zgrasratn_(:,:,:) * tmask(:,:,:) )
      CALL iom_put("zepsherv_mes", zepsherv_(:,:,:) * tmask(:,:,:) )
      CALL iom_put("zepsherf_mes", zepsherf_(:,:,:) * tmask(:,:,:) )
      CALL iom_put("zepshert_mes", zepshert_(:,:,:) * tmask(:,:,:) )
      CALL iom_put("zepsherq_mes", zepsherq_(:,:,:) * tmask(:,:,:) )
      CALL iom_put("zbeta_mes" , zbeta_(:,:,:) * tmask(:,:,:) )
      CALL iom_put("zgrazpof_mes" , zgrazpof_(:,:,:) * tmask(:,:,:) )
      CALL iom_put("zproport_mes" , zproport_(:,:,:) * tmask(:,:,:) )
      CALL iom_put("zgrasratf_mes", zgrasratf_(:,:,:) * tmask(:,:,:) )
      CALL iom_put("zgraztotf_mes" , zgraztotf_(:,:,:) * tmask(:,:,:) )
      CALL iom_put("zgraztotc_mes" , zgraztotc_(:,:,:) * tmask(:,:,:) )
      CALL iom_put("zgraztotn_mes" , zgraztotn_(:,:,:) * tmask(:,:,:) )
      ...
      ENDIF  ! ************************** End of the ln_dvm_meso part ****************************************
