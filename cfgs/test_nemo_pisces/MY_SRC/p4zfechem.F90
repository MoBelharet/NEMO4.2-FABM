...
CONTAINS
   ...
   SUBROUTINE p4z_fechem( kt, knt, Kbb, Kmm, Krhs )
      ...
      REAL(wp), DIMENSION(jpi,jpj,jpk) ::   zcoll3d, zscav3d, zlcoll3d, zdust_
      REAL(wp), DIMENSION(jpi,jpj,jpk) ::  zkeq_diag, zklight_diag, zconsfe_diag, za1_diag, ztfe_diag
      !!---------------------------------------------------------------------
      !
      ...
      DO_3D( nn_hls, nn_hls, nn_hls, nn_hls, 1, jpkm1)
         ...
         zkeq_diag(ji,jj,jk)= zkeq  
         zklight_diag(ji,jj,jk) = zklight
         zconsfe_diag(ji,jj,jk) = zconsfe
         za1_diag(ji,jj,jk) = za1
         ztfe_diag(ji,jj,jk) = ztfe

      END_3D

      CALL iom_put("zkeq", zkeq_diag(:,:,:) * tmask(:,:,:) )
      CALL iom_put("zklight", zklight_diag(:,:,:) * tmask(:,:,:) )
      CALL iom_put("zconsfe", zconsfe_diag(:,:,:) * tmask(:,:,:) )
      CALL iom_put("za1", za1_diag(:,:,:) * tmask(:,:,:) )
      CALL iom_put("ztfe", ztfe_diag(:,:,:) * tmask(:,:,:) )
      

      !
      plig(:,:,:) =  MAX( 0., ( zFeL1(:,:,:) / ( tr(:,:,:,jpfer,Kbb) + rtrn ) ) )

      CALL iom_put("plig" , plig(:,:,:) * tmask(:,:,:) )
      ...
      CALL iom_put("zdust" , zdust_(:,:,:) * tmask(:,:,:) )
      ...
      IF( lk_iomput .AND. knt == nrdttrc ) THEN
         ...
         IF( iom_use("FESCAV") )  CALL iom_put("FESCAV" , zscav3d(:,:,:)  * 1e9 * tmask(:,:,:)  /xstep )
         IF( iom_use("FECOLL") )  CALL iom_put("FECOLL" , zcoll3d(:,:,:)  * 1e9 * tmask(:,:,:) /xstep )
