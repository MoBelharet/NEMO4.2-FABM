...
CONTAINS
   ...
   SUBROUTINE p4z_bc( kt, Kbb, Kmm, Krhs )
      ...
      REAL(wp), DIMENSION(jpi,jpj,jpk) :: zrivdin_diag, zdustdep_diag
      REAL(wp), DIMENSION(jpi,jpj) :: zndep_no3, zndep_nh4
      !
      ...
      IF( ll_dust )  THEN
         ...
         IF ( ln_trc_sbc(jpfer) ) THEN
            DO_3D( nn_hls, nn_hls, nn_hls, nn_hls, 2, jpkm1 )
               ...
               zdustdep_diag(ji,jj,jk ) = zdustdep
               !
         ...
         CALL iom_put("zdustdep", zdustdep_diag(:,:,:) * tmask(:,:,:) )
      ENDIF
      ...
      IF( ll_river ) THEN
          ...
          DO_2D( nn_hls, nn_hls, nn_hls, nn_hls )
             DO jk = 1, nk_rnf(ji,jj) 
                ...
                zrivdin_diag(ji,jj,jk) = zrivdin
            ENDDO
          END_2D
      ENDIF
      CALL iom_put("zrivdin", zrivdin_diag(:,:,:) * tmask(:,:,:) )
      ...
      IF( ll_ndepo ) THEN
         ...
         IF( ln_trc_sbc(jpnh4) ) THEN
            ...
            DO_2D( nn_hls, nn_hls, nn_hls, nn_hls )
               zndep = rf_trsfac(jl) * sf_trcsbc(jl)%fnow(ji,jj,1) / e3t(ji,jj,1,Kmm) / rn_sbc_time
               zndep_nh4(ji,jj) = zndep
               tr(ji,jj,1,jptal,Krhs) = tr(ji,jj,1,jptal,Krhs) + rno3 * zndep * rfact
            END_2D
         ENDIF
      ENDIF
      CALL iom_put("zndep_no3", zndep_no3(:,:) * tmask(:,:,1) )
      CALL iom_put("zndep_nh4", zndep_nh4(:,:) * tmask(:,:,1) )
      !
      ...
      IF( ln_ironice ) THEN
         ...
         IF( lk_iomput ) THEN
          ...
          CALL iom_put( "fmmflx" , fmmflx(:,:) * tmask(:,:,1) )
         ENDIF
      ...
      IF( ln_ironsed .AND. .NOT.lk_sed ) THEN
          ...
          CALL iom_put("e3t_init"  , e3t_0(:,:,:)  * tmask(:,:,:) )
      ENDIF
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'nampisbc in configuration namelist' )
      ...
      IF( ln_ironsed ) THEN     
         ...
         !CALL iom_put("zcmask" , zcmask(:,:,:) * tmask(:,:,:) )
