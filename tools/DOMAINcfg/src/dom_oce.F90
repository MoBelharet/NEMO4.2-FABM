MODULE dom_oce
  ...
  !! time & space domain namelist
   ...
   INTEGER , PUBLIC ::   nn_hmin_2d        !: = 0/1 constant or read from the bathymetry file
...
!!gm
   ...
   REAL(wp) , PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:) ::   hmin_2d             !: number of ocean level (=0, 1, ... , jpk-1)
   ...
   INTEGER FUNCTION dom_oce_alloc()
    ...
    !     &                      hu_a(jpi,jpj) , hv_a(jpi,jpj) , r1_hu_a(jpi,jpj) , r1_hv_a(jpi,jpj) , STAT=ierr(6)  )
      ...
      ALLOCATE( gdept_1d(jpk) , e3tp (jpi,jpj), e3wp(jpi,jpj) ,gdepw_1d(jpk) , e3t_1d(jpk) , e3w_1d(jpk) , STAT=ierr(6) )
         !
      ALLOCATE( bathy(jpi,jpj), hmin_2d(jpi,jpj), mbathy(jpi,jpj), tmask_i(jpi,jpj) , tmask_h(jpi,jpj) ,                        &
