...
!            CALL lbc_lnk( 'zgr_bat', bathy, 'T', 1._wp )
      ...
      ELSEIF( ntopo > 0 ) THEN                       !   read in file   ! (over the local domain)
         ...
         IF( ln_zps .OR. ln_sco )   THEN              ! zps or sco : read meter bathymetry
            ...
            IF( ntopo == 1) THEN
               ...
               IF (nn_hmin_2d == 1) THEN
                  IF(lwp) WRITE(numout,*) "Reading hmin field from bathymetry file", cn_topo
                  CALL iom_get  ( inum, jpdom_auto, "hmin", hmin_2d )
               END IF
               CALL iom_close( inum )
            ELSE
               CALL dom_bat
      ...
      IF ( .not. ln_sco ) THEN
         ...
         IF (nn_hmin_2d == 1) THEN
            ...
            IF(lwp) write(numout,*) 'Minimum ocean depth is set using hmin_2d read from bayhymetry '
            WHERE( bathy(:,:) <= 0.0_wp ) bathy(:,:) = 0._wp                         ! min=0     over the lands
            WHERE( bathy(:,:) >  0.0_wp ) bathy(:,:) = MAX(  hmin_2d(:,:), bathy(:,:)  )   ! min=zhmin over the oceans
