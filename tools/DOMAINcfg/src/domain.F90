...
CONTAINS
   ...
   SUBROUTINE dom_nam
      ...
      NAMELIST/namdom/ ln_read_cfg, nn_bathy, cn_domcfg, cn_topo, cn_bath, cn_lon, cn_lat, rn_scale, nn_interp, &
         ...
         &             rn_bathy , rn_e3zps_min, rn_e3zps_rat, nn_msh, rn_hmin, nn_hmin_2d,                      &
...
!         nn_bathy = Agrif_Parent(nn_bathy)
      ...
      IF(lwp) THEN
         ...
         IF (nn_hmin_2d == 1 ) THEN
            WRITE(numout,*) ' nn_hmin_2d = ', nn_hmin_2d
            WRITE(numout,*) ' so min depth of the ocean is read from bathymetry file (hmin)'
         ELSE
