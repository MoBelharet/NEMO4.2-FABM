...
!  LOGICAL , PUBLIC ::   ln_dynldf_iso   !: iso-neutral direction                        (see ldfslp)
   ...
   LOGICAL,  PUBLIC  ::   ln_2dcsmg = .FALSE.  !: Smagorinsky constant of proportionality
   ...
   REAL(wp),         ALLOCATABLE, SAVE, DIMENSION(:,:)   ::   csmg_2d      !: Square of the local gridscale (e1e2/(e1+e2))**2
...
CONTAINS
   ...
   SUBROUTINE ldf_dyn_init
      ...
      NAMELIST/namdyn_ldf/ ln_dynldf_OFF, nn_dynldf_typ, ln_dynldf_lap, ln_dynldf_blp,   &   ! type of operator
         ...
         &                 rn_csmc      , rn_minfac    , rn_maxfac, ln_2dcsmg                           ! Smagorinsky settings
      !!----------------------------------------------------------------------
      !
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namdyn_ldf in configuration namelist' )
      ...
      IF(lwp) THEN                      ! Parameter print
         ...
         WRITE(numout,*) '         2d Smagorinsky coefficient read from file smg.nc ln_2dcsmg       = ', ln_2dcsmg
      ...
      ELSE                                   !==  a lateral diffusion operator is used  ==!
         ...
         CASE(  32  )       !==  time varying 3D field  ==!
            ...
            IF (ln_2dcsmg) THEN
               ALLOCATE(csmg_2d(jpi,jpj), STAT=ierr )
               IF( ierr /= 0 )   CALL ctl_stop( 'STOP', 'ldf_dyn_init: failed to allocate Smagorinsky csmg_2d')
               IF(lwp) WRITE(numout,*) '   ==>>>   Read smagorinsky = csmg_2d(i,j)  in smg.nc file'
               CALL iom_open( 'smg.nc', inum )
               CALL iom_get ( inum, jpdom_global, 'smg', csmg_2d)
               CALL iom_close( inum )
            ...
            DO jk = 1, jpkm1
              !
               ...
               IF (ln_2dcsmg) THEN
                  ...
                  !IF (ssumask(ji,jj) == 1 ) PRINT *, 'csmg ', csmg_2d(ji,jj), mig0(ji), mjg0(jj)

                  zcmsmag   = (csmg_2d(ji,jj)/rpi)**2                                            ! (C_smag/pi)^2
                  zstabf_lo = rn_minfac * rn_minfac / ( 2._wp * 12._wp * 12._wp * zcmsmag ) ! lower limit stability factor scaling
                  zstabf_up = rn_maxfac / ( 4._wp * zcmsmag * 2._wp * rn_Dt )               ! upper limit stability factor scaling
                  IF( ln_dynldf_blp ) zstabf_lo = ( 16._wp / 9._wp ) * zstabf_lo          ! provide |U|L^3/12 lower limit instead 
