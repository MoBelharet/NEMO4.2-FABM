MODULE sbcblk
   ...
   REAL(wp)         ::   rn_cdfac = 1.0, rn_lwadd  = 0.0  ! Saeed Falahat adding amplication and adding factor for the wind and long wave radiation 
   !
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namsbc_blk in configuration namelist' )
      ...
      IF(lwp) THEN                     !** Control print
         ...
         WRITE(numout,*) '      wind drag amplification                             rn_cdfac     = ', rn_cdfac
         
         WRITE(numout,*) '      long wave radiation addition                        rn_lwadd     = ', rn_lwadd
   ...
   SUBROUTINE blk_oce_2( ptair, pdqlw, pprec, psnow, &   ! <<= in
      ...
      zqlw(:,:) = (zqlw(:,:) + rn_lwadd ) * tmask(:,:,1)   ! Long  Wave
      ! ----------------------------------------------------------------------------- !
      !     IV    Total FLUXES                                                       !
      ! ----------------------------------------------------------------------------- !
      !
