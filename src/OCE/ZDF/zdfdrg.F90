MODULE zdfdrg
   ...
   LOGICAL          ::   ln_z0_2d     !: =T read 2d z0 field from bfr_roughness.nc; =F use rn_z0
   ...
   !
   LOGICAL          ::   l_log_z0_2d        !: flag to use 2d bottom roughness field
   ...
   REAL(wp), ALLOCATABLE, SAVE, DIMENSION(:,:), PUBLIC ::   rz0_top, rz0_bot     !: 2d bottom roughness at t-
...
CONTAINS
   ...
   SUBROUTINE zdf_drg( kt, Kmm, k_mk, pCdmin, pCdmax, pz0, pke0, pCd0,pz0_2d,   &   ! <<== in 
      ...
      REAL(wp), DIMENSION(:,:), INTENT(in   ) ::   pz0_2d   ! 2d bottom roughness length field [m]
      REAL(wp), DIMENSION(:,:), INTENT(  out) ::   pCdU     ! = - Cd*|U|   (t-points) [m/s]
      !!
      INTEGER ::   ji, jj   ! dummy loop indices
      INTEGER ::   imk      ! local integers
      REAL(wp)::   zzz, zut, zvt, zcd, z0   ! local scalars
      !!----------------------------------------------------------------------
      !
      IF( l_log_not_linssh ) THEN     !==  "log layer"  ==!   compute Cd and -Cd*|U|
         DO_2D_OVR( nn_hls-1, nn_hls-1, nn_hls-1, nn_hls-1 )
            ...
            !
            IF( l_log_z0_2d ) THEN ! use 2d z0 field
               z0 = pz0_2d(ji,jj)
            ELSE
               z0 = pz0
            ENDIF
!!JC: possible WAD implementation should modify line below if layers vanish
            ...
            IF (zzz == 0.0 .OR. z0 <= 0.0 .OR. (zzz / z0) == 1.0) THEN
...
902   IF( ios >  0 )   CALL ctl_nam( ios , 'namdrg in configuration namelist' )
      ...
      !
      ALLOCATE( rCd0_bot(jpi,jpj), rCdU_bot(jpi,jpj),rz0_bot(jpi,jpj) )
      CALL drg_init( 'BOTTOM'   , mbkt       ,                                         &   ! <== in
         &           r_Cdmin_bot, r_Cdmax_bot, r_z0_bot, r_ke0_bot, rCd0_bot, rCdU_bot,rz0_bot)   ! ==> out
      !
      ...
      !
      IF( ln_isfcav ) THEN
         ALLOCATE( rCd0_top(jpi,jpj),rz0_top(jpi,jpj))
         CALL drg_init( 'TOP   '   , mikt       ,                                         &   ! <== in
            &           r_Cdmin_top, r_Cdmax_top, r_z0_top, r_ke0_top, rCd0_top, rCdU_top,rz0_top)   ! ==> out
      ENDIF
      !
   ...
   SUBROUTINE drg_init( cd_topbot, k_mk,  &
      &                 pCdmin, pCdmax, pz0, pke0, pCd0, pCdU,pz0_2d ) 
      !!----------------------------------------------------------------------
      !!                  ***  ROUTINE drg_init  ***
      !!
      ...
      !!
      REAL(wp), DIMENSION(:,:), INTENT(  out) ::   pz0_2d          ! 2d bottom roughness length field [m]
      CHARACTER(len=40) ::   cl_namdrg, cl_file, cl_varname, cl_namref, cl_namcfg  ! local names
      CHARACTER(len=40) ::   cl_file_z0, cl_varname_z0  ! local names
      INTEGER ::   ji, jj              ! dummy loop indexes
      ...
      !!
      NAMELIST/namdrg_top/ rn_Cd0, rn_Uc0, rn_Cdmax, rn_ke0, rn_z0, ln_z0_2d, ln_boost, rn_boost
      NAMELIST/namdrg_bot/ rn_Cd0, rn_Uc0, rn_Cdmax, rn_ke0, rn_z0, ln_z0_2d, ln_boost, rn_boost
      !!----------------------------------------------------------------------
      !
      ...
      CASE( 'TOP   ' )
         ...
         cl_file    = 'tfr_coef.nc'
         cl_varname = 'tfr_coef'
         cl_file_z0    = 'tfr_roughness.nc'
         cl_varname_z0 = 'tfr_roughness'
      ...
      CASE( 'BOTTOM' )
         ...
         cl_file    = 'bfr_coef.nc'
         cl_varname = 'bfr_coef'
         cl_file_z0    = 'bfr_roughness.nc'
         cl_varname_z0 = 'bfr_roughness'
      CASE DEFAULT
...
902   IF( ios >  0 )   CALL ctl_nam( ios , TRIM(cl_namcfg) )
      ...
      CASE( np_loglayer )       !== logarithmic layer formulation of friction  ==!   (CdU = (vkarman log(z/z0))^2 |U| )
         ...
         l_zdfdrg = .TRUE.          !* Cd*|U| updated at each time-step (it depends on ocean velocity)
         !
         IF( ln_z0_2d ) THEN  !* read 2D roughness to pz0_2d variable
            IF(lwp) WRITE(numout,*)
            IF(lwp) WRITE(numout,*) '   ==>>>   use a regional roughness read in ', TRIM(cl_file_z0), ' file,'
            IF(lwp) WRITE(numout,*) '           roughness value defined in namelist is ignored '
            ! cl_varname is 2d bottom roughness lenth field (m)
            CALL iom_open ( TRIM(cl_file_z0), inum )
            CALL iom_get  ( inum, jpdom_global, TRIM(cl_varname_z0), pz0_2d, 1 )
            CALL iom_close( inum )
            l_log_z0_2d = .TRUE.    !* Use 2d z0 field when computing Cd
         ENDIF
