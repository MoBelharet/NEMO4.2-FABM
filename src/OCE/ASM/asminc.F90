MODULE asminc
...
#if defined key_si3
   USE ice      , ONLY : a_i, h_i, h_s, v_i, v_s, a_i_b, h_i_b, h_s_b, v_i_b, v_s_b, at_i, at_i_b, hm_i
#endif
   !
   ...
   PUBLIC   ice_asm_inc    !: Apply the seaice increment
   ...
   LOGICAL, PUBLIC :: ln_iceinc     !: Logical switch for sea ice concentration increment
   ...
   REAL(wp), PUBLIC, DIMENSION(:,:), ALLOCATABLE ::   conc_bkg, conc_bkginc ! Background sea ice concentration and its increment
   REAL(wp), PUBLIC, DIMENSION(:,:), ALLOCATABLE ::   hice_bkg, hice_bkginc ! Background sea ice thickness and its increment
   REAL(wp), PUBLIC, DIMENSION(:,:), ALLOCATABLE ::   hsnw_bkg, hsnw_bkginc ! Background snow thickness and its increment
...
CONTAINS
   ...
   SUBROUTINE asm_inc_init( Kbb, Kmm, Krhs )
      ...
      NAMELIST/nam_asminc/ ln_bkgwri,                                      &
         &                 ln_trainc, ln_dyninc, ln_sshinc, ln_iceinc,     &
         &                 ln_seaiceinc, ln_asmdin, ln_asmiau, ln_temnofreeze,  &
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'nam_asminc in configuration namelist' )
      ...
      IF(lwp) THEN
         ...
         WRITE(numout,*) '      Logical switch for applying ice/snow increments          ln_iceinc = ', ln_iceinc
      ...
      IF (      ( ( .NOT. ln_asmdin ).AND.( .NOT. ln_asmiau ) ) &
           .AND.( ( ln_trainc ).OR.( ln_dyninc ).OR.( ln_sshinc ) .OR. ( ln_seaiceinc) .OR. ( ln_iceinc) )) &
      ...
      ALLOCATE( conc_bkginc  (jpi,jpj)     )   ;   conc_bkginc  (:,:)   = 0._wp
      ALLOCATE( hice_bkginc  (jpi,jpj)     )   ;   hice_bkginc  (:,:)   = 0._wp
      ALLOCATE( hsnw_bkginc  (jpi,jpj)     )   ;   hsnw_bkginc  (:,:)   = 0._wp
      ...
      IF ( ln_trainc .OR. ln_dyninc .OR.   &       !--------------------------------------
         & ln_sshinc .OR. ln_seaiceinc .OR. ln_iceinc  ) THEN    ! Read the increments from file
         !                                         !--------------------------------------
         ...
         IF ( ln_seaiceinc ) THEN
            CALL iom_get( inum, jpdom_auto, 'bckinconc', seaice_bkginc, 1 )
            ! Apply the masks
         ...
         IF ( ln_iceinc ) THEN
            ...
            CALL iom_get( inum, jpdom_auto, 'bckinconc', conc_bkginc, 1 )
            ! Apply the masks
            conc_bkginc(:,:) = conc_bkginc(:,:) * tmask(:,:,1)
            ! Set missing increments to 0.0 rather than 1e+20
            ! to allow for differences in masks
            WHERE( ABS( conc_bkginc(:,:) ) > 1.0e+10 ) conc_bkginc(:,:) = 0.0

            CALL iom_get( inum, jpdom_auto, 'bckinhice', hice_bkginc, 1 )
            ! Apply the masks
            hice_bkginc(:,:) = hice_bkginc(:,:) * tmask(:,:,1)
            ! Set missing increments to 0.0 rather than 1e+20
            ! to allow for differences in masks
            WHERE( ABS( hice_bkginc(:,:) ) > 1.0e+10 ) hice_bkginc(:,:) = 0.0

            CALL iom_get( inum, jpdom_auto, 'bckinhsnw', hsnw_bkginc, 1 )
            ! Apply the masks
            hsnw_bkginc(:,:) = hsnw_bkginc(:,:) * tmask(:,:,1)
            ! Set missing increments to 0.0 rather than 1e+20
            ! to allow for differences in masks
            WHERE( ABS( hsnw_bkginc(:,:) ) > 1.0e+10 ) hsnw_bkginc(:,:) = 0.0
...
!!gm why not e3u(:,:,:,Kbb), e3v(:,:,:,Kbb), gdept(:,:,:,Kbb) ????
   ...
   SUBROUTINE ice_asm_inc( kt, kindic )
      !!----------------------------------------------------------------------
      !!                    ***  ROUTINE ice_asm_inc  ***
      !!          
      !! ** Purpose : Apply the ice and snow assimilation increments.
      !!
      !! ** Method  : Direct initialization or Incremental Analysis Updating.
      !!
      !! ** Action  : 
      ...
      IF ( ln_asmiau ) THEN
         ...
         IF ( ( kt >= nitiaustr_r ).AND.( kt <= nitiaufin_r ) ) THEN
            ...
            IF(lwp) THEN
               WRITE(numout,*) 
               WRITE(numout,*) 'ice_asm_inc : ice IAU at time step = ', &
                  &  kt,' with IAU weight = ', wgtiau(it)
               WRITE(numout,*) '~~~~~~~~~~~~'
            ENDIF
            ...
            ! add ice concentration increments
	    a_i(:,:,1) = a_i(:,:,1) + conc_bkginc(:,:) * zincwgt
            a_i(:,:,1) = MIN( MAX( a_i(:,:,1), 0.0_wp), 1.0_wp)

            ! add ice thicknes increments
	    h_i(:,:,1) = h_i(:,:,1) + hice_bkginc(:,:) * zincwgt
            h_i(:,:,1) = MIN( MAX( h_i(:,:,1), 0.0_wp), 9.99_wp)

            ! add snow thickness increments
	    h_s(:,:,1) = h_s(:,:,1) + hsnw_bkginc(:,:) * zincwgt
            h_s(:,:,1) = MIN( MAX( h_s(:,:,1), 0.0_wp), 9.99_wp)

            ! recalculate ice and snow volumes
            v_i (:,:,1) = h_i (:,:,1) * a_i (:,:,1)
            v_s (:,:,1) = h_s (:,:,1) * a_i (:,:,1)

            ! Update the before fields
            a_i_b(:,:,1) = a_i(:,:,1)
            h_i_b(:,:,1) = h_i(:,:,1)
            h_s_b(:,:,1) = h_s(:,:,1)
            v_i_b(:,:,1) = v_i(:,:,1)
            ...
            ndaice_da(:,:) = conc_bkginc(:,:) * zincwgt / rdt
#endif

            IF ( kt == nitiaufin_r ) THEN
               DEALLOCATE( conc_bkginc )
               DEALLOCATE( hice_bkginc )
               DEALLOCATE( hsnw_bkginc )
            ENDIF
      ...
      ELSEIF ( ln_asmdin ) THEN
         ...
         IF ( kt == nitdin_r ) THEN
            ...
            ! add ice concentration increment and constrain to [0:1]
            a_i(:,:,1) = conc_bkg(:,:)  + conc_bkginc(:,:)
            a_i(:,:,1) = MIN( MAX( a_i(:,:,1), 0.0_wp), 1.0_wp)

            ! add ice thickness increment and constrain to [0:9.99]
            h_i(:,:,1) = hice_bkg(:,:) + hice_bkginc(:,:)
            h_i(:,:,1) = MIN( MAX( h_i(:,:,1), 0.0_wp), 9.99_wp)

            ! add snow thickness increment and constrain to [0:9.99]
            h_s(:,:,1) = hsnw_bkg(:,:) + hsnw_bkginc(:,:)
            h_s(:,:,1) = MIN( MAX( h_s(:,:,1), 0.0_wp), 9.99_wp)
            ...
            ! recalculate ice and snow volumes
            ! Note: The following leads to a crash
            v_i (:,:,:) = h_i (:,:,:) * a_i (:,:,:)
            ! v_i(:,:,1) = a_i(:,:,1) * h_i(:,:,1)
            ! v_s(:,:,1) = a_i(:,:,1) * h_s(:,:,1)
