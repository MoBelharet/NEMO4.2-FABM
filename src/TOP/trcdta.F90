...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namtrc_dta_ini in configuration namelist' )
   ...
   SUBROUTINE trc_dta_all( kt, ntrc, ptrcdta)
      !!----------------------------------------------------------------------
      !!                   ***  ROUTINE trc_dta  ***
      !!
      !! ** Purpose :   provides passive tracer data at kt
      ...
      INTEGER                          , INTENT(in   )   ::   kt         ! ocean time-step
      INTEGER                          , INTENT(in   )   ::   ntrc       ! number of tracers
      REAL(wp),  DIMENSION(jpi,jpj,jpk,jptra), INTENT(inout  ) ::   ptrcdta    ! 3D data array
      !
      INTEGER ::   ji, jj, jk, jl, jn, jkk, ik    ! dummy loop indices
      REAL(wp)::   zl, zi
      REAL(wp), DIMENSION(jpk) ::  ztp                ! 1D workspace
      CHARACTER(len=100) :: clndta
      !!----------------------------------------------------------------------
      ...
      IF( nb_trcdta > 0 ) THEN
         ...
         DO jn = 1, ntrc
            IF( ln_trc_ini(jn) ) THEN
               jl = n_trc_index(jn)
               ptrcdta(:,:,:,jn) = sf_trcdta(jl)%fnow(:,:,:) * tmask(:,:,:)
            ENDIF
         !
#if ! defined key_sed_off
            IF( ln_sco ) THEN                !== s- or mixed s-zps-coordinate  ==!
               ...
               DO_2D( nn_hls, nn_hls, nn_hls, nn_hls )                 ! vertical interpolation of T & S
                  DO jk = 1, jpk                        ! determines the intepolated T-S profiles at each (i,j) points
                     zl = gdept_0(ji,jj,jk)
                     IF(     zl < gdept_1d(1  ) ) THEN         ! above the first level of data
                        ztp(jk) = ptrcdta(ji,jj,1,jn)
                     ELSEIF( zl > gdept_1d(jpk) ) THEN         ! below the last level of data
                        ztp(jk) = ptrcdta(ji,jj,jpkm1,jn)
                     ELSE                                      ! inbetween : vertical interpolation between jkk & jkk+1
                        DO jkk = 1, jpkm1                                  ! when  gdept_1d(jkk) < zl < gdept_1d(jkk+1)
                           IF( (zl-gdept_1d(jkk)) * (zl-gdept_1d(jkk+1)) <= 0._wp ) THEN
                              zi = ( zl - gdept_1d(jkk) ) / (gdept_1d(jkk+1)-gdept_1d(jkk))
                              ztp(jk) = ptrcdta(ji,jj,jkk,jn) + ( ptrcdta(ji,jj,jkk+1,jn) - ptrcdta(ji,jj,jkk,jn) ) * zi
                           ENDIF
                        END DO
                     ENDIF
                  END DO
                  DO jk = 1, jpkm1
                     ptrcdta(ji,jj,jk,jn) = ztp(jk) * tmask(ji,jj,jk)     ! mask required for mixed zps-s-coord
                  END DO
                  ptrcdta(ji,jj,jpk,jn) = 0._wp
               END_2D
               !
            ELSE                                !==   z- or zps- coordinate   ==!
               ! zps-coordinate (partial steps) interpolation at the last ocean level
               IF( ln_zps ) THEN
                   DO_2D( nn_hls, nn_hls, nn_hls, nn_hls )
                      ...
                      IF( ik > 1 .AND. gdept_0(ji,jj,ik) < gdept_1d(ik) ) THEN
                         zl = ( gdept_1d(ik) - gdept_0(ji,jj,ik) ) / ( gdept_1d(ik) - gdept_1d(ik-1) )
                         ptrcdta(ji,jj,ik,jn) = (1.-zl) * ptrcdta(ji,jj,ik,jn) + zl * ptrcdta(ji,jj,ik-1,jn)
                      ENDIF
                      ik = mikt(ji,jj)
                      IF( ik > 1 ) THEN
                         zl = ( gdept_0(ji,jj,ik) - gdept_1d(ik) ) / ( gdept_1d(ik+1) - gdept_1d(ik) )
...
#endif
            ! Scale by multiplicative factor
            ptrcdta(:,:,:,jn) = ptrcdta(:,:,:,jn) * rf_trfac(jl)
            !
         ENDDO
      ENDIF
      !
