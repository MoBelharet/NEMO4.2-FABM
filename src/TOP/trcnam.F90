...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namtrc in configuration namelist' )
   ...
   SUBROUTINE trc_nam_trc
      ...
      !!
#if defined key_fabm
      NAMELIST/namtrc/jp_bgc, ln_fabm, ln_pisces, ln_my_trc, ln_age, ln_cfc11, ln_cfc12, ln_sf6, ln_c14, &
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namtrc in configuration namelist' )
      ...
      IF( ln_fabm   .AND. ln_my_trc )   CALL ctl_stop( 'Choose only ONE BGC model - FABM or MY_TRC' )

      IF( .NOT. ln_pisces .AND. .NOT. ln_my_trc  .AND. .NOT. ln_fabm  )   jp_bgc = 0
      ll_cfc = ln_cfc11 .OR. ln_cfc12 .OR. ln_sf6
      !
      jptra       =  0
      jp_fabm     =  0    ;   jp_fabm0  =  0   ;   jp_fabm1 = 0      
