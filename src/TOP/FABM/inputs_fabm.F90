...
#  include "domzgr_substitute.h90"
  ...
  199   RETURN
   ...
   SUBROUTINE trc_rnf_fabm( kt , Kmm, Krhs)
      !!----------------------------------------------------------------------
              ...
              DO ji = ntsi,ntei !fs_2, fs_jpim1
                ...
                DO jk = 1,nk_rnf(ji,jj)
                  ...
                  if (river_data%rn_trrnfac>=0) then
                    ...
                    tr(ji,jj,jk,river_data%jp_pos, Krhs) = tr(ji,jj,jk,river_data%jp_pos, Krhs) + river_data%sf(1)%fnow(ji,jj,1)*zcoef
                  else
                    ...
                    tr(ji,jj,jk,river_data%jp_pos, Krhs) = tr(ji,jj,jk,river_data%jp_pos, Krhs) + (river_data%sf(1)%fnow(ji,jj,1)/1000._wp) * tr(ji,jj,jk,river_data%jp_pos, Kmm) / h_rnf(ji,jj)
                  endif
#if defined key_trdtrc && defined key_iomput
