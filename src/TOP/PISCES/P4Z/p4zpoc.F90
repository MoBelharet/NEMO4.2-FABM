...
CONTAINS
   ...
   SUBROUTINE p4z_poc( kt, knt, Kbb, Kmm, Krhs )
      ...
      REAL(wp), DIMENSION(jpi,jpj,jpk)   :: zremipoc, zremigoc, zorem3, ztremint, zfolimi, zpoc_g, zpoc_, zsizek_, zsizek_g
     ...
     CALL iom_put('gom_zpoc', zpoc_g(:,:,:) * tmask(:,:,:) )
     CALL iom_put('gom_expz', zsizek_g(:,:,:) * tmask(:,:,:) )

     !-----------/ Mokrane /-----------------------------------------------------
     DO jn = 1, jcpoc
        write (strindex, '(i0)') jn
        CALL iom_put('gom_alpha' // trim(strindex) , alphag(:,:,:,jn) * tmask(:,:,:) )         
     END DO
     !-----------------------------------------------------------------------------
      ...
      CALL iom_put('ztremint_goc', ztremint(:,:,:) * tmask(:,:,:) )
     ...
     CALL iom_put('pom_prod', prodpoc(:,:,:) * tmask(:,:,:) /xstep )
     CALL iom_put('gom_prod', prodgoc(:,:,:) * tmask(:,:,:) /xstep )
     ...
     CALL iom_put('pom_zpoc', zpoc_(:,:,:) * tmask(:,:,:) )
     
     CALL iom_put('pom_expz', zsizek_(:,:,:) * tmask(:,:,:) )

    !-----------/ Mokrane /-----------------------------------------------------
      DO jn = 1, jcpoc
          write (strindex, '(i0)') jn
          CALL iom_put('pom_alpha' // trim(strindex) , alphap(:,:,:,jn) * tmask(:,:,:) )
      END DO
    !-----------------------------------------------------------------------------
     ...
     CALL iom_put('ztremint_poc', ztremint(:,:,:) * tmask(:,:,:) )
