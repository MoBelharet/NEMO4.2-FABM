...
CONTAINS
   ...
   SUBROUTINE p4z_flx ( kt, knt, Kbb, Kmm, Krhs )
      ...
      REAL(wp), DIMENSION(jpi,jpj) ::   zkgco2, zkgo2, zh2co3, zoflx,  zpco2atm, zpco2oce , zfld16_, zflu16_ 
      !!---------------------------------------------------------------------
      !
      ...
      DO_2D( nn_hls, nn_hls, nn_hls, nn_hls )
         ...
         ! ---- Mokrane ------
         zfld16_(ji,jj) = zfld16 * tmask(ji,jj,1)
         zflu16_(ji,jj) = zflu16 * tmask(ji,jj,1)

      END_2D
      !----- Mokrane -----
      CALL iom_put( "zfld16"  , zfld16_(:,:)* 1000.  ) 
      CALL iom_put( "zflu16"  , zflu16_(:,:)* 1000.  )
      !-----------------
