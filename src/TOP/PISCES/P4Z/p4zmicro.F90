...
CONTAINS
   ...
   SUBROUTINE p4z_micro( kt, knt, Kbb, Krhs )
      ...
      REAL(wp), DIMENSION(jpi,jpj,jpk) :: zgrazing, zfezoo, zgrasratf_z, zgraztotf_z
      ...
      DO_3D( nn_hls, nn_hls, nn_hls, nn_hls, 1, jpkm1)
         ...
         zgrasratf_z(ji,jj,jk) = zgrasratf
         zgraztotf_z(ji,jj,jk) = zgraztotf/xstep
      ...
      CALL iom_put("zgrasratf_zoo", zgrasratf_z(:,:,:) * tmask(:,:,:) )
      CALL iom_put("zgraztotf_zoo", zgraztotf_z(:,:,:) * tmask(:,:,:) )

      !
      IF( lk_iomput .AND. knt == nrdttrc ) THEN
        IF( iom_use("GRAZ1") ) THEN  !   Total grazing of phyto by zooplankton
           zgrazing(:,:,jpk) = 0._wp   ; CALL iom_put( "GRAZ1" , (zgrazing(:,:,:)/xstep) * tmask(:,:,:) )!* 1.e+3  * rfact2r * tmask(:,:,:) ) 
         ENDIF
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namp4zzoo in configuration namelist' )
      ...
      IF(lwp) THEN                         ! control print
         ...
         WRITE(numout,*) '      Epsilon                                         rtrn        =', rtrn
